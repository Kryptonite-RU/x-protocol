...
def load_src_data(filename):
    jstr = file_to_json(filename)
    src = json_to_src(jstr)
    return src

def load_usr_data(filename):
    jstr = file_to_json(filename)
    usr = json_to_usr(jstr)
    return usr

def load_insp_data(filename):
    jstr = file_to_json(filename)
    insp = json_to_insp(jstr)
    return insp

def save_src_data(filename, src):
    jstr = src_to_json(src)
    json_to_file(filename, jstr)

def save_usr_data(filename, usr):
    jstr = usr_to_json(usr)
    json_to_file(filename, jstr)

def save_insp_data(filename, insp):
